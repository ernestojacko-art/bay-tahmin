...
def double_chance_from_1x2(ox: OneXTwoProbabilities) -> DoubleChance:
    """Çifte şans: 1X2'nin doğrudan toplamı, yeni veri gerektirmez."""
    return DoubleChance(
        home_or_draw=round(ox.home_win + ox.draw, 4),
        draw_or_away=round(ox.draw + ox.away_win, 4),
        home_or_away=round(ox.home_win + ox.away_win, 4),
    )


def draw_no_bet_from_1x2(ox: OneXTwoProbabilities) -> DrawNoBet:
    """Beraberlik olasılığı çıkarılıp ev/deplasman arasında yeniden normalize edilir."""
...
def asian_handicap_lines(
...
) -> list[AsianHandicapLine]:
    """
    Asya handikapı, aynı Poisson/Dixon-Coles skor matrisinden türetilir
    (ev sahibi takıma uygulanan çizgi). Tam sayı çizgilerde (-1, 0, 1 gibi)
    berabere kalma ("push") olasılığı ayrıca raporlanır; yarım çizgilerde
    (-0.5, 0.5 gibi) push matematiksel olarak imkansızdır.
    """
    ...
    for line in lines:
        ...
        for s in matrix:
            ...
            if diff > 1e-9:
                home_cover += s.probability
            elif diff < -1e-9:
                away_cover += s.probability
            else:
                push += s.probability
        results.append(
            AsianHandicapLine(
