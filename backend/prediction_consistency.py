...
The presentation layer must never receive contradictory probabilities. Score,
FT 1X2, BTTS, O/U 2.5, HT and HT/FT are reconciled from one probability system
whenever the required expected-goal priors are available. Missing inputs are
...
def _expected_goals(model: dict[str, Any], context: dict[str, Any]) -> tuple[float, float] | None:
    blocks = []
    for name in ("goal_model", "score_model", "poisson", "goal_model_proxy", "score_projection"):
    ...
    for block in blocks:
        ...
        if isinstance(expected, dict):
            ...
            if home is not None and away is not None:
                return home, away
        for hk, ak in (("home", "away"), ("home_lambda", "away_lambda"), ("lambda_home", "lambda_away"), ("home_xg", "away_xg")):
...
def _half_lambdas(model: dict[str, Any], context: dict[str, Any], second: bool) -> tuple[float, float] | None:
    block = model.get("second_half_model" if second else "first_half_model") or {}
    expected = block.get("expected_goals") if isinstance(block, dict) else None
    if isinstance(expected, dict):
        home = _num(expected.get("home")); away = _num(expected.get("away"))
        if home is not None and away is not None:
            return home, away
...
def reconcile(model: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Reconcile all published prediction families and attach validation diagnostics."""
    ...
    if goals:
        ...
        checks.append("Skor → MS/BTTS/Üst-Alt aynı skor dağılımından türetildi")
    ...
    if first_half and second_half:
        ...
        result["iyms"] = {
            ...
            "surprise_candidates": [
                {"selection": key, "probability": round(value * 100, 2)}
                for key, value in sorted(joint.items(), key=lambda item: item[1], reverse=True)
                if key not in {"1/1", "X/X", "2/2"}
            ][:5],
        }
        result["htft_model"] = {
            "independent_first_half": True,
            "independent_second_half": True,
            "joint_method": "HT matrix × 2H matrix",
            "consistency_locked": True,
        }
        checks.append(f"İY={top_ht}; İY/MS={top_joint}; iki sonuç aynı joint dağılımdan geliyor")
    else:
        # If independent priors are unavailable, do not invent a joint result.
        result.setdefault("prediction_warnings", []).append("Bağımsız İY ve 2Y öncülleri birlikte mevcut değil; HT/MS joint tahmini üretilmedi")
    ...
    result["prediction_consistency"] = {
        "validated": True,
        "score_ft_linked": bool(score_matrix),
        "htft_linked": bool(first_half and second_half),
        "checks": checks,
    }
    return result
...
def install(impl: Any) -> None:
    """Wrap the active v6 candidate function without rewriting historical engine files."""
